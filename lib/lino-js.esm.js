...
class Query {
    ...
    getOngoingProposalList() {
        ...
        return this._transport.querySubspace(Keys$1.getOngoingProposalPrefix(), ProposalKVStoreKey, GetKeyBy.GetSubstringAfterSubstore);
    }
    /**
    ...
    getExpiredProposalList() {
        const ProposalKVStoreKey = Keys$1.KVSTOREKEYS.ProposalKVStoreKey;
        return this._transport.querySubspace(Keys$1.getExpiredProposalPrefix(), ProposalKVStoreKey, GetKeyBy.GetSubstringAfterSubstore);
    }
    /**
    ...
    getOngoingProposal(proposalID) {
        const ProposalKVStoreKey = Keys$1.KVSTOREKEYS.ProposalKVStoreKey;
        return this._transport.query(Keys$1.getOngoingProposalKey(proposalID), ProposalKVStoreKey);
    }
    /**
    ...
    getExpiredProposal(proposalID) {
        const ProposalKVStoreKey = Keys$1.KVSTOREKEYS.ProposalKVStoreKey;
        return this._transport.query(Keys$1.getExpiredProposalKey(proposalID), ProposalKVStoreKey);
    }
    /**
    ...
    getGlobalTime() {
        const GlobalKVStoreKey = Keys$1.KVSTOREKEYS.GlobalKVStoreKey;
        return this._transport.query(Keys$1.getTimeKey(), GlobalKVStoreKey);
    }
    /**
    ...
    getInterest(username) {
        return new Promise((resolve, reject) => {
            this.getVoter(username)
                ...
                this.getGlobalTime()
                    .then(globalTime => {
                    var pastDay = (Number(voter.last_power_change_at) - Number(globalTime.chain_start_time)) /
                        (3600 * 24);
                    pastDay = pastDay > 0 ? pastDay : 0;
                    var currentDay = (Number(globalTime.last_block_time) - Number(globalTime.chain_start_time)) /
                        (3600 * 24);
                        ...
                        if (allInterest != null) {
                            allInterest.forEach(stat => {
                                if (stat !== null && Number(stat.unclaimed_lino_power) > 0) {
                                    interest += Number(((Number(voter.lino_stake) / Number(stat.unclaimed_lino_power)) *
                                        Number(stat.unclaimed_friction)).toFixed(5));
                                }
                            });
                        }
                    });
    ...
    getLinoStakeStat(day) {
        const GlobalKVStoreKey = Keys$1.KVSTOREKEYS.GlobalKVStoreKey;
        return this._transport.query(Keys$1.getLinoStakeStatKey(day), GlobalKVStoreKey);
    }
    /**
     * getEventAtTime returns the events at certain second.
     */
