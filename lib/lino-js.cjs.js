...
function decodeObject(result) {
    ...
    if ('delegated_power' in decodedResult) {
        decodedResult.delegated_power.amount = String(Number(decodedResult.delegated_power.amount) / 100000);
    }
    ...
    if ('total_report_stake' in decodedResult) {
        decodedResult.total_report_stake.amount = String(Number(decodedResult.total_report_stake.amount) / 100000);
    }
    if ('total_upvote_stake' in decodedResult) {
        decodedResult.total_upvote_stake.amount = String(Number(decodedResult.total_upvote_stake.amount) / 100000);
    }
    ...
    if ('app_consumption' in decodedResult) {
        decodedResult.app_consumption.amount = String(Number(decodedResult.app_consumption.amount) / 100000);
    }
    ...
    if ('transaction_capacity' in decodedResult) {
        decodedResult.transaction_capacity.amount = String(Number(decodedResult.transaction_capacity.amount) / 100000);
    }
    if ('original_income' in decodedResult) {
        decodedResult.original_income.amount = String(Number(decodedResult.original_income.amount) / 100000);
    }
    if ('friction_income' in decodedResult) {
        decodedResult.friction_income.amount = String(Number(decodedResult.friction_income.amount) / 100000);
    }
    if ('actual_reward' in decodedResult) {
        decodedResult.actual_reward.amount = String(Number(decodedResult.actual_reward.amount) / 100000);
    }
    if ('unclaim_reward' in decodedResult) {
        decodedResult.unclaim_reward.amount = String(Number(decodedResult.unclaim_reward.amount) / 100000);
    }
    ...
    if ('disagree_vote' in decodedResult) {
        decodedResult.disagree_vote.amount = String(Number(decodedResult.disagree_vote.amount) / 100000);
    }
    var keys = Object.keys(result);
    for (var index in keys) {
        var key = keys[index];
        if ((key === 'details' || key === 'frozen_money_list' || key === 'donation_list') &&
...
class Broadcast {
     ...
     */
    register(referrer, register_fee, username, resetPubKey, transactionPubKeyHex, micropaymentPubKeyHex, postPubKeyHex, referrerPrivKeyHex, seq) {
        const msg = {
     ...
     * @param seq: the sequence number of user for the next transaction
     */
    recover(username, new_reset_public_key, new_transaction_public_key, new_micropayment_public_key, new_post_public_key, privKeyHex, seq) {
        const msg = {
...
class Query {
    ...
    getPostAllLikes(author, postID) {
        const PostKVStoreKey = Keys$1.KVSTOREKEYS.PostKVStoreKey;
        const Permlink = Keys$1.getPermlink(author, postID);
        return this._transport.querySubspace(Keys$1.getPostLikePrefix(Permlink), PostKVStoreKey, exports.GetKeyBy.GetSubstringAfterKeySeparator);
    }
    /**
